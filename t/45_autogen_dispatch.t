...
# ---- karr #71 (P5): a nested $ref to ANOTHER autogenerated definition -----
#
# _schema_to_type_spec's $ref branch recursively calls get_or_generate() for
# a $ref that points at a second, non-GVK definition (as opposed to the
# apimachinery specials -- IntOrString/Quantity/Time -- or the built-in
# ObjectMeta $ref every GVK class already exercises via karr #56/#60). This
# had never been exercised end to end through $k8s->inflate(): does the
# nested field actually come back a typed, accessor-bearing AutoGen object,
# not a bare hashref?

subtest 'nested $ref to another AutoGen definition inflates to a typed object (karr #71 / P5)' => sub {
    my $nested_spec = {
        definitions => {
            'io.example.v1.Sprocket' => {
                type => 'object',
                'x-kubernetes-group-version-kind' => [
                    { group => 'example.com', version => 'v1', kind => 'Sprocket' },
                ],
                properties => {
                    spec => { '$ref' => '#/definitions/io.example.v1.SprocketSpec' },
                },
            },
            # Not itself a GVK/top-level Kind -- exactly the shape a
            # sub-object definition referenced by $ref normally has.
            'io.example.v1.SprocketSpec' => {
                type       => 'object',
                properties => {
                    size  => { type => 'string' },
                    teeth => { type => 'integer' },
    ...
    my $k8s = IO::K8s->new(openapi_spec => $nested_spec, resource_map => {});

    my $obj = eval {
        $k8s->inflate({
            apiVersion => 'example.com/v1',
            kind       => 'Sprocket',
            metadata   => { name => 'sprocket-1' },
            spec       => { size => 'large', teeth => 12 },
        });
    };
    ok($obj, 'inflate succeeds') or diag $@;

    SKIP: {
        skip 'inflate failed, cannot assert on the nested $ref field', 6 unless $obj;

        my $ns = $k8s->_autogen_namespace;
        isa_ok($obj, "${ns}::io::example::v1::Sprocket", 'top-level Sprocket object');
        is($obj->metadata->name, 'sprocket-1', 'metadata inflated');

        my $spec_obj = $obj->spec;
        ok(IO::K8s::AutoGen::is_autogen(ref $spec_obj) || 0,
            'spec resolved to an AutoGen-generated class, not left as a bare hashref')
            or diag('ref($obj->spec) = ' . (ref($spec_obj) || '<not a ref>'));
        isa_ok($spec_obj, "${ns}::io::example::v1::SprocketSpec",
            'spec is the SprocketSpec definition specifically')
            if ref($spec_obj);

        SKIP: {
            skip 'spec did not resolve to an object with accessors', 2
                unless ref($spec_obj) && $spec_obj->can('size');
            is($spec_obj->size, 'large', 'nested $ref field size is typed and inflated, not a hash key');
            is($spec_obj->teeth, 12, 'nested $ref field teeth is typed and inflated, not a hash key');
        }
    }
};
