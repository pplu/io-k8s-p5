...
This cluster-scoped resource provides per-node BGP configuration overrides that take precedence over cluster-level BGP settings, enabling fine-grained node-specific BGP behavior. It uses API version C<cilium.io/v2>. The C<spec> and C<status> fields contain opaque CRD-specific data structures managed by the Cilium BGP control plane controller.
